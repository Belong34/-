...
class Zed3D:
    ...
    def set_true_football(self, xs, ys, zs):
        self.ture_footballx = xs
        self.ture_footbally = ys
        self.ture_footballz = zs

    def true_traking_draw(self):
        if len(self.ture_footballx) > 1:
            glPushMatrix()
            glBegin(GL_LINE_STRIP)
            for i in range(1, len(self.ture_footballx)):
                glColor3f(0.7, 0.7, 0.7)
                glVertex3f(self.ture_footballx[i - 1], self.ture_footbally[i - 1], self.ture_footballz[i - 1])
                glVertex3f(self.ture_footballx[i], self.ture_footbally[i], self.ture_footballz[i])
                # glVertex3f(self.path_mem[i].get()[0], self.path_mem[i].get()[1], self.path_mem[i].get()[2])
            glEnd()
            glPopMatrix()

    def set_football(self, xs, ys, zs):
        self.footballx.clear()
        self.footbally.clear()
        self.footballz.clear()
        self.footballx = xs
        self.footbally = ys
        self.footballz = zs
        # print(self.footbally)

    def traking_draw(self):
        if len(self.footballx) > 1:
            glPushMatrix()
            glBegin(GL_LINE_STRIP)
            for i in range(1, len(self.footballx)):
                glColor3f(0.5, 0.5, 0.5)
                glVertex3f(self.footballx[i - 1], self.footbally[i - 1], self.footballz[i - 1])
                glVertex3f(self.footballx[i], self.footbally[i], self.footballz[i])
                # glVertex3f(self.path_mem[i].get()[0], self.path_mem[i].get()[1], self.path_mem[i].get()[2])
            glEnd()
            glPopMatrix()
